...
Entity.prototype.update = function() {
  ...
  if (this.vx > .25  && this.vy > .25) {
  this.vx -= Constants.FRICTION * this.deltaTime / 1000;
  this.vy -= Constants.FRICTION * this.deltaTime / 1000;
  } else if (this.vx < -.25 && this.vy < -.25){
  this.vx += Constants.FRICTION * this.deltaTime / 1000;
  this.vy += Constants.FRICTION * this.deltaTime / 1000;
  }else{
  this.vx = this.vy = 0;
  ...
  console.log('Position: ' + this.x + ',' + this.y + 'Vel: ' + this.vx + ', ' + this.vy);
};

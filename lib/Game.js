 ...
 * @param {Object} data An object containing the client's input. It should
 *   contain the keys keyboardState, mouse, and click.
 */
Game.prototype.updatePlayer = function(id, data) {
  var player = this.players.get(id);
  var context = this;
  if (player) {
    player.updateOnInput(data.keyboardState, data.mouse, data.click, function () {
        context.makeBomb(xPos, yPos, xTar, yTar id);
    });
  }
};

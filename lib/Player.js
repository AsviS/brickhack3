...
/**
 * The minimum time the player must wait between bomb tosses.
 * @const
 * @type {number}
 */
...
Player.prototype.updateOnInput = function(keyboardState, mouse, click, cb) {
  ...
  if (click && currentTime > this.bombCooldown + this.lastBombTime) {
    ...
    /**
     * @dan
     * this.throwBomb is redundant, just put the functionality in here
     */
  }
};
 ...
 * @param {function} addBombCallback Callback that adds a bomb
 * */
Player.prototype.throwBomb = function(x, y, addBombCallback) {
  addBombCallback(this.x, this.y, x, y, this.id);
};

...
Bomb.create = function(x, y, targetX, targetY, orientation) {
  var vx = Util.getSign(targetX - x) * Math.sqrt(2 * Constants.FRICTION * Math.abs(targetX - x));
  var vy = Util.getSign(targetY - y) * Math.sqrt(2 * Constants.FRICTION * Math.abs(targetY - y));
  return new Bomb(x, y, vx, vy, orientation);
};

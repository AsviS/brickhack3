...
Bomb.create = function(x, y, targetX, targetY) {
  var vx = Utils.getSign(targetX - this.x) * Math.sqrt(2 * Constants.FRICTION * Math.abs(targetX - this.x));
  var vy = Utils.getSign(targetY - this.y) * Math.sqrt(2 * Constants.FRICTION * Math.abs(targetY - this.y));
  return new Bomb(x, y, vx, vy);
};

Bomb.prototype.update = function() {
  this.parent.update.call(this);
  console.log("[" + (new Date()).toString() + "] " + "Bomb(x=" + this.vx + ", y=" + this.vy + ')');
};
